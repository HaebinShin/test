...
words = [word.strip() for word in open("./email_words.txt", 'r').readlines()]
most_common_words = [word.strip() for word in open("./email_most_common_words.txt", 'r').readlines()]
g_filler_words = words[:]
...
class Function():
	...
	def generate(self, depth, total_nodes, num_comps, MAX_DEPTH, MAX_TOTAL):
		...
		for can_gen_candidates in self.children_type_candidates():
			...
			num_comps += 1 if (isinstance(next_thing, AndComp) or isinstance(next_thing, OrComp) or isinstance(next_thing, NotComp)) else 0
...
def random_string():
	...
	if r < 0.8:
		...
		ran_str = random.choice(g_filler_words)
	return ran_str
...
class BaseFunction(Function):
	...
	def description(self, tryit=False):
		...
		try:
			...
			#filler_words = ["dog", "truck", "ring", "lake"]
			self.filler_words = random.sample(g_filler_words, 4)
...
'''
	...
	def get_base_positive_example(self):
		...
		#filler_words = ["dog", "truck", "ring", "lake"]
		filler_words = random.sample(g_filler_words, 4)
...
class CharacterFunc(Literal):
	...
	def logical_form(self):
		#return "."
		return "{}".format(self.rand_char)
	def description(self):
		self.rand_char = random.choice(string.ascii_letters)
		return "a character '{}'".format(self.rand_char)
...
class MostCommonRandWordFunc(Literal):
    def __init__(self, *args):
        Function.__init__(self, *args)
        self.real_word = random.choice(most_common_words)
        self.word = "WORD"
...
class AndComp(Composition):
	...
	def logical_form(self):
		#return "({})&({})".format(self.children[0].logical_form(),self.children[1].logical_form())
		return "((.*{}.*).*(.*{}.*))|((.*{}.*).*(.*{}.*))".format(self.children[0].logical_form(),self.children[1].logical_form(),
                self.children[1].logical_form(), self.children[0].logical_form())
	...
	def children_type_candidates(self):
		#return [ANY, ANY]
		return [ANY_FOR_AND, ANY_FOR_AND]
...
type_dict["Literal"] = [NumberFunc,  RandWordFunc, RandWordFunc, RandWordFunc, VowelsFunc]
type_dict["LiteralForAnd"] = [MostCommonRandWordFunc, CharacterFunc]
type_dict["LiteralForNot"] = [MostCommonRandWordFunc]
#type_dict["Modifier"] = [OneOrMoreMod, NOrMoreMod, ZeroOrMoreMod, ContainsOnlyFunc]
type_dict["Modifier"] = [OneOrMoreMod, ZeroOrMoreMod, ContainsOnlyFunc]
...
ANY_FOR_AND = type_dict["Function"] + type_dict["Composition"], type_dict["Modifier"] + type_dict['LiteralForAnd']
ANY_FOR_NOT = type_dict["Function"] + type_dict['LiteralForNot']
...
def is_a_good_func(func):
	if not func or isinstance(func, str):
		return False
...
def test_generation(print_format='pretty'):
	...
	params_list = [(1, 3, 10000), (2, 4, 10000)]
